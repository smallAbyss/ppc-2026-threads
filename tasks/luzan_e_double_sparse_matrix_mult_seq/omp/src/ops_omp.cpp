#include "luzan_e_double_sparse_matrix_mult_seq/seq/include/ops_omp.hpp"
...
SparseMatrix LuzanEDoubleSparseMatrixMultOMP::CalcProdOMP(const SparseMatrix &a, const SparseMatrix &b) const {
    ...
    std::vector<std::vector<unsigned>> rows_per_col(b.cols_);
      ...
      #pragma omp parallel for shared(a, b) schedule(static) default(none) num_threads(2)
